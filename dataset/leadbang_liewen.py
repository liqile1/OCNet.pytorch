...
def get_train_list(root):
    path = osp.join(root, 'train.txt')
    ...
    for aa in a:
        try:
          ...
          pass
    # b = [int(aa[:-1]) for aa in a]
    return b

def get_test_list(root=None):
    return list(range(1129, 1589))
...
class LeadBangTrain(data.Dataset):
    def __init__(self, root, max_iters=None,
        ...
        self.cache_label = {}
        self.item_idx_list = []
        train_idx_list = get_train_list(root)
        for item_idx in train_idx_list:
...
class LeadBangTest(data.Dataset):
    def __init__(self, root, max_iters=None,
        ...
        self.cache_label = {}
        self.item_idx_list = []
        test_idx_list = get_test_list()
        for item_idx in test_idx_list:
            #print(item_idx)
            self.item_idx_list.append(item_idx)
            img_path = 'source/' + str(item_idx) + ".bmp"
            label_path = 'label/' + str(item_idx) + ".bmp"
